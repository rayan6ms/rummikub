...
class RummikubSolver:
    ...
    def __init__(self, tiles, sets, numbers=13, colours=4, rack=[], table=[]):
        ...
        self.value = np.array([v for c in range(colours)
        ...
        self.table_array = np.array(
            [self.table.count(self.tiles[i]) for i in range(len(self.tiles))])
        self.rack_array = np.array(
            [self.rack.count(self.tiles[i]) for i in range(len(self.tiles))])
    ...
    def solve(self, maximise='tiles', initial_meld=False):
        ...
        return prob.value, prob.solution.primal_vars[list(prob.solution.primal_vars.keys())[0]], \
            prob.solution.primal_vars[list(
                prob.solution.primal_vars.keys())[1]]
