...
def group_contours(sorted_contours, group_distance=25):
    groups = []
    current_group = []
    last_x_end = 0
    last_y_center = None
...
def predict_images():
    ...
    for img_name in sorted(os.listdir(extracted_numbers), key=lambda x: (int(x.split("_")[-1].split(".")[0]), int(x.split("_")[0]))):
        if img_name.endswith(".png"):
